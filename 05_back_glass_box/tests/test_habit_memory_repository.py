...
# Agregar la ruta development al path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'development'))

# Importaciones usando rutas absolutas desde development
from finance.core.domain.habit import Habit
from finance.adapters.outbound.db_memory.habit_memory_repository import HabitMemoryRepository
...
class TestHabitMemoryRepository:
    ...
    def test_get_scheduled_days_count_various_configurations(self, sample_habits):
        ...
        # Habit with scheduled days (Jan 2024: 15 days - Mon, Wed, Fri)
        scheduled_count = sample_habits[0].get_scheduled_days_count()
        assert scheduled_count == 14  # 5 weeks * 3 days = 15
